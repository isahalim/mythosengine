/**
 * "Gradient Orb" by @uicapsule on 21st.dev, vendored **verbatim** on
 * 2026-09-04 by operator direction ("use DIRECTLY THE EXACT one from
 * 21st.dev").
 *
 *   https://21st.dev/@uicapsule/components/gradient-orb
 *
 * Everything below the imports is the component as published: the GLSL, the
 * YIQ hue rotation, the hash-based 3D simplex noise, the three base colours,
 * the breathing pulse, the fullscreen-triangle geometry, and the
 * `GradientOrbConfig` surface. It is not adapted, restyled or simplified —
 * the same standing arrangement `src/app/glass/**` has with "Broken by
 * Design", and for the same reason: a vendored component that has been
 * quietly improved is a component nobody can diff against its source.
 *
 * **The one deviation, and it is a deletion.** Upstream carries two
 * `eslint-disable-next-line react-hooks/exhaustive-deps` comments. This repo
 * has no react-hooks plugin, so those directives name a rule that does not
 * exist here and ESLint fails on them — they are removed, and the
 * explanatory comments beside them are kept. That is the reverse of what
 * CLAUDE.md's NEVER block forbids: nothing is being silenced, a silencer that
 * silences nothing is being taken out.
 *
 * **What it costs and why that is contained.** This is the only part of this
 * project that pulls in `three` and `@react-three/fiber` — roughly 170 KB
 * gzipped against an app-JS budget of 110 KB for everything else. So it is
 * never imported directly: `OrbLazy.tsx` is the only entry point, it is
 * `React.lazy`, and `astro.config.mjs` gives it a chunk of its own so
 * size-limit can hold the two budgets apart. The landing, the passkey
 * ceremony and the entire brainstorm route download none of it.
 *
 * **What it is, in this product.** The design board asked for a black hole
 * that gathers the glass shards and makes them orbit; the operator replaced
 * that with this orb on 2026-09-04, and it also replaces the board's aurora.
 * `src/app/chat/StageBuilding.tsx` is what animates it out of the text input
 * and up the screen. Nothing about that lives in this file.
 */
"use client";
