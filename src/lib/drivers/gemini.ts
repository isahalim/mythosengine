...
/**
 * Gemini LLM driver — **RESEARCH's first attempt, and nothing else**
 * (operator direction, 2026-09-02).
 *
 * Read the scope before reaching for this. It drove RESEARCH, reranking,
 * SCRIPT, PLAN and EDIT for a few hours on 2026-09-01 and was reverted the
 * same day, because a free tier metering **5 requests/minute per model**
 * cannot carry a pipeline that runs three reasoning stages back to back.
 * That reasoning has not changed. What changed is that RESEARCH alone —
 * the one stage whose quality is bounded by how much source text it can
 * hold — gets one bounded attempt here before falling to Groq, and the
 * attempt is capped at four turns precisely so the per-minute ceiling is
 * never approached. Everything else on the reasoning path stays on Groq:
 * see `src/config/models.ts` and `src/lib/rag/research-provider.ts`.
 *
 * The value being bought is intake, not intelligence. Groq's path has to
 * fit a whole tool conversation inside 7,200 tokens and drops retrieved
 * material to do it (`fitToRequestBudget`); this one does not have to drop
 * anything, so four turns here see more grounding than six turns there.
 *
 * Tool calling is the reason `providerSteps` exists — Gemini cannot replay
 * a tool conversation statelessly. See `LlmMessage.providerSteps`.
 */
export class GeminiLlmDriver implements LlmDriver {
  ...
  constructor(private readonly options: GeminiDriverOptions) {
    ...
    // Longer than Groq's 10s. RESEARCH's later turns carry the whole
    // retrieved corpus — full article bodies, not the 6,000-character
    // truncations the Groq path can afford — and a request that size takes
    // longer to be read than it does to be answered.
