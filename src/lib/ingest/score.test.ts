...
describe("scoreObservedSignals", () => {
  ...
  it("promotes distinct signals with no near-duplicates to scored", () => {
    ...
    const result = scoreObservedSignals(ctx.db, now);
  ...
  it("collapses a 3-way near-duplicate cluster to exactly one scored survivor", () => {
  ...
  it("applies a corroboration bonus proportional to cluster size", () => {
    insertSignal({ id: "a", title: "GTA 6 trailer breaks YouTube view count record", engagementScore: 0.5 });
    ...
    scoreObservedSignals(ctx.db, now);
  ...
  it("rejects a future-dated signal outright, even with the highest engagementScore", () => {
    insertSignal({
      id: "future",
  ...
  it("a future-dated signal never wins its cluster over a valid duplicate", () => {
    insertSignal({
      id: "future",
  ...
  it("counts corroboration against already-scored signals from earlier runs in the trailing window", () => {
    insertSignal({ id: "earlier", title: "GTA 6 trailer breaks YouTube view count record" });
    scoreObservedSignals(ctx.db, now); // promotes "earlier" to scored
