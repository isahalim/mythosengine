...
- **Two model providers, split by job** (operator direction 2026-09-01). **Gemini** does the reasoning — RESEARCH (tool-calling over BM25 plus a live source-fetch tool), retrieval **reranking** (`src/lib/rag/rerank.ts`), SCRIPT, PLAN, and EDIT's tool loop — on a *ladder*: `gemini-3.7-flash` → `3.6-flash` → `3.5-flash` → `3.5-flash-lite` (`src/lib/drivers/gemini-ladder.ts`). The free tier meters **5 requests/minute and 250K tokens/day per model**, so descending the ladder buys a genuinely separate budget rather than merely retrying; RENDER also pauses 61s between consecutive Gemini stages so the per-minute window can refill. **Groq** (`openai/gpt-oss-120b`, `openai/gpt-oss-20b` — replacing `llama-3.3-70b-versatile`/`llama-3.1-8b-instant`, both deprecated by Groq 2026-06-17) keeps **CRITIC**, deliberately: a critic sharing a model with the writer it judges is grading its own work. Groq is also the **fallback** for every Gemini stage when the whole ladder is spent, so a bad quota day costs quality, not the video — and the audit package records which provider actually answered. **That is the entire list.** With no `GEMINI_API_KEY` set, every stage runs on Groq exactly as it did before. The in-console chat and voice agents were deleted 2026-08-31 with the console that hosted them, so the Worker itself now makes no model call at all. Footage acquisition used to run a browser agent here (on `qwen/qwen3.8-27b`); it was deleted on 2026-08-29 once the converter flow was shown to be deterministic, so FOOTAGE REFRESH now spends no tokens at all
...
- Never let a Gemini stage fail the render when Groq could answer it. RESEARCH, SCRIPT and PLAN all fall back (`withGroqFallback`), and the fallback is recorded, never silent. Likewise never let EDIT fail a render: a missing `uvx`, a missing Kinocut, or a spent ladder means "every clip as sourced", flagged.
- Never reconstruct a Gemini tool-calling turn from `content` + `toolCalls`. Its `thought` steps carry signatures that must be echoed verbatim, and a rebuilt turn is rejected with a bare `invalid_request` that names nothing. Carry `LlmMessage.providerSteps` through untouched — measured against the live API, 2026-09-01.
