...
- **One model provider for reasoning: Groq `openai/gpt-oss-120b`** (operator direction 2026-09-01), named once in `src/config/models.ts`. It answers RESEARCH (tool-calling over BM25 plus a live source-fetch tool), retrieval **reranking** (`src/lib/rag/rerank.ts`), SCRIPT, CRITIC, PLAN, EDIT's Kinocut loop, and title/description/hashtag generation. **The smaller `openai/gpt-oss-20b` is not used** — RESEARCH and PLAN ran on it to keep their tokens out of the 120b model's separate 200K/day, and that split is gone by operator direction, so the whole reasoning path now shares one daily budget. (Both replaced `llama-3.3-70b-versatile`/`llama-3.1-8b-instant`, deprecated by Groq 2026-06-17.) **Gemini is narration only** — `GEMINI_API_KEY` buys the TTS upgrade and nothing else. It drove the reasoning stages for a few hours on 2026-09-01 and was reverted the same day: the free tier meters 5 requests/minute *per text model*, RESEARCH's tool loop alone can spend six, and the first live run peaked at 6/5 on `gemini-3.7-flash` and lost the render at SCRIPT. The ladder, `withGroqFallback` and `LlmMessage.providerSteps` were deleted with it; a per-minute ceiling a normal render cannot stay under is a dependency, not an upgrade. **That is the entire list.** The in-console chat and voice agents were deleted 2026-08-31 with the console that hosted them, so the Worker itself now makes no model call at all. Footage acquisition used to run a browser agent here (on `qwen/qwen3.8-27b`); it was deleted on 2026-08-29 once the converter flow was shown to be deterministic, so FOOTAGE REFRESH now spends no tokens at all
