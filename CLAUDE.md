...
- **Groq** (`openai/gpt-oss-120b`, `openai/gpt-oss-20b` — replacing `llama-3.3-70b-versatile`/`llama-3.1-8b-instant`, both deprecated by Groq 2026-06-17) for script generation, critique, and metadata (title/description/hashtags). The footage browser agent runs on **`qwen/qwen3.8-27b`** instead (operator directive, 2026-08-29): Groq's limits are per-model and its binding constraint is tokens-per-**day**, where qwen3.8-27b's 2M dwarfs gpt-oss's 200K. `groq/compound` was considered and is unusable here — it supports no custom tools, only Groq's own built-ins. Any model driving the browser agent must support OpenAI-compatible custom function calling
...
- **Browser-driven footage acquisition** (ARCHITECTURE.md §5.0): the weekly footage-refresh job drives a real headless Chromium (Playwright). **Search is deterministic** (`youtube-search-dom.ts`) — it reads youtube.com's results for `"<game>" walkthrough "<channel>" youtube` straight off the page and costs zero tokens; only the convert+download leg via `https://media.ytmp3.gg/tools/youtube-to-mp4-converter/dbismy` uses a bounded tool-calling loop (`browser-agent-core.ts`), because that part genuinely varies. Spend the model where there is real ambiguity, nowhere else. Replaces yt-dlp and the YouTube Data API v3 search — both removed. No upload/OAuth scope exists anywhere in this system — publishing is manual, done by the operator in YouTube Studio
