...
**A passkey is bound to the host, so moving the Worker invalidates it (2026-09-04).** `src/server/router.ts`'s `rpConfigFor` derives the RP ID from the request's own hostname, which is why nothing in the code had to change when the account subdomain was renamed `5ryfrrjgmg` → `isahalim` — and exactly why the operator's registered passkey stopped working the moment it did: the browser will not offer a credential created under a different RP ID, and `verifyAuthenticationResponse` would reject it on `expectedRPID` if it did. Nothing in the `credentials` table records which host a row belongs to, so a stale row is indistinguishable from a live one except by `last_used_at`.

The recovery path is the enrollment token, and it is narrow by design: `/auth/passkey/register/begin` needs no session (it is what establishes one), but `MAX_REGISTERED_CREDENTIALS` is 2, so a host move is survivable only while a slot is free. **Before moving this Worker again: have `CONSOLE_ENROLLMENT_TOKEN` in hand, and delete the stale credential row afterwards** — leaving it consumes the spare slot, and the second move would find enrollment closed with no way to open it but a database write. Done in that order on 2026-09-04; the count is back to 1 of 2.
