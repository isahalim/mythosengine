...
- `scripts.beats` holds the beats; `scripts.body` holds the flattened spoken narration, so AUDIT SUMMARY's near-duplicate check, the export package and the console's review queue read a script without knowing what a beat is. Both columns stay nullable for rows written before the format landed.
- **The v1 prose path is gone** (deleted 2026-09-03). `generateScript`, `ScriptResponseSchema`, `prompts/script.v1.md`, `prompts/script.v2.md` and `schemas/script.schema.json` were retained "for callers that predate the format" and no caller ever did — RENDER has called `generateDiscourseScript` since the format landed, so the prose path was reachable only from its own tests while still carrying a second prompt, a second schema and a nullable `beats`/`targetDurationS` through every consumer downstream.
- `prompts/script.v3.md` receives the signal's title/summary **and the RESEARCH brief** — and nothing else. That is the same hallucination boundary v1 drew, just around a larger set of *retrieved, cited* facts instead of around the title alone: rule 7 tells the writer the research block is everything it knows, and that an angle is its to invent while a fact is not. A null brief renders as an explicit "no research was available" line, never as a blank section — a blank one reads to a model like an oversight to fill in from memory, which is the exact failure RESEARCH exists to prevent.
- **The structure gate fails a render; the length estimate does not** (2026-09-03). `validateBeatStructure` marks each violation `fatal` or `advisory`. Fatal is the format itself — no `land`, no `pushback`, or a `pushback` outside the `attempt`…`land` window — and after two attempts it fails the stage, because shipping a lecture would ship the exact format this plan replaced. Advisory is the ±25% read-time band, and it may not: `estimatedReadSeconds` is a single 165-wpm constant standing in for delivery speed, pauses and `ttsRateRange`, and on 2026-09-03 it killed a finished render over `118s is over the 113s ceiling for a 90s video`, taking a completed RESEARCH brief with it. A draft whose only remaining complaint is its length is **accepted**, reported in `GeneratedScript.structureNotes`, and flagged for the operator by AUDIT SUMMARY's word-count check — which now computes its bounds from `wordCountRange` in discourse.ts rather than its own copy of the constants, so the gate and the flag cannot drift apart.
- **Both length rejections quote the word count to aim at**, not only the seconds missed by, and the closer of two drafts is the one kept. The live failure was an oscillation: attempt one under the floor, attempt two over the ceiling, and the old loop scored only the last draft, so the better of the two was discarded unlooked-at.
