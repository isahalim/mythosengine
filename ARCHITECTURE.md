...
- **The meters are per minute as well as per request, and a wait is not a failure.** The morning fix of 2026-09-04 read only half the qwen3 output ceiling. The other half is an ordinary rolling meter: under the per-request wall, a legal request is still refused once the minute is spent — `429 ... on output tokens per minute (OTPM): Limit 1000, Used 702, Requested 321. Please try again in 1.38s`, and Groq reserves *its own estimate* of the answer, not our `max_tokens`, so nothing local can predict it. The evening render met it, `fetchWithRetry`'s 5s default ceiling on one sleep turned `Retry-After 10s` into a `rate_limited` error, `LadderLlmDriver` spent both rungs on it inside 70 seconds, and shots 3 through 7 were abandoned without a request ever being made for them — 0 of 8 clips edited. The pipeline's Groq driver now waits up to 60s (`PIPELINE_MAX_RETRY_DELAY_MS`), which every `Retry-After` measured that day fits inside; the job's own budget is 180 minutes. The shared limiter also paces against the **lowest** input ceiling in play, since qwen3's ITPM is 7,000 where gpt-oss's is 8,000 (`QUOTAS.groq.inputTokensPerMinuteQwen3`) — and their own response header advertises 8,000.
- **EDIT asks for no reasoning at all.** `reasoningEffort: "none"`. The work is "probe, detect, trim": every decision is read off a tool result. A hidden reasoning trace is billed against the same `max_tokens` *and* the same 1,000-a-minute meter, and on `qwen/qwen3.6-27b` — measured live at 122-173 tokens on a first turn, more with a scene list in the transcript — it ran the turn out of budget, which arrives as `finish_reason: "length"` with **empty content and no tool call**. That is indistinguishable from a model that declined to answer, and it is what "the model finished without naming a final file path" meant for shots 0 and 1 of the same render.
- **A wasted turn costs a turn, not the clip.** A turn with neither a tool call nor a `FINAL:` line now gets one corrective ask and the loop continues; only the last turn ends the clip. Three separate things land there and a second ask fixes all of them: a truncation, a turn that narrates the plan instead of executing it, and a tool call the *provider* rejected before it ran (`tool call validation failed: parameters for tool video_trim did not match schema`), which arrives as the prose the model wrote before the call with the call itself gone. When Groq reports that specific rejection the ask names it, because it is a thing the model can fix.
...
- **The model's output path is checked to exist** before the encoder is told to read it. A hallucinated path would otherwise fail the whole render at encode time, long after the stage that invented it. **When there is no usable path in the model's answer, Kinocut's own is used**: `video_trim` returns the path it wrote, so a clip that was actually cut ships as cut whether or not the sentence naming it survived. Shot 0 of the evening render called `video_trim` twice and still shipped as sourced; the same recovery covers a model that closes with a path it invented over the one the tool handed it, watched live the same day.
